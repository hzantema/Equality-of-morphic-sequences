...
begin
maak;
writeln('\noindent We will prove the following ',chib,' properties simultaneously by induction on $n$.'); 
writeln;
for j := 1 to chib do
begin
write('(',j-1,') $\tau(f^n(');
...
if not fout then writeln('\noindent Basis of induction proved.');
if not fout then
for j := 1 to chib do
begin
writeln;
...
begin 
t := t + 1; 
if ck[j,t-1] then write(')) \tau(f^n(',f[1,c[1,j,i],k]) else write(f[1,c[1,j,i],k]); 
end;
